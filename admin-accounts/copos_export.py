...
from database import _fetchall, _fetchone, PH
    ...
    for member in members:
        ...
        for i in range(6):
            if i < len(payments):
                ...
                total_dues_paid += du
                pmt_cols.extend([
                    pmt_date, format_currency(eq),
                    format_currency(du), format_currency(eq + du)
                ])
            else:
        ...
        row = [
            str(member["member_number"]), member["name_1"],
            member["name_2"] or "", member["street_address"],
            ...
            paid_in_installments, initial_payment_date,
            format_currency(member["signup_fee"]), installment_fee_paid,
