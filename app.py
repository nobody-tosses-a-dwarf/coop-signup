...
from database import get_db, init_db, seed_chatham, create_member
...
async def home(request: Request):
    conn = get_db()
    coops = conn.execute("SELECT slug, name, city, state FROM coops ORDER BY name").fetchall()
    conn.close()
...
async def signup_form(request: Request, slug: str):
    conn = get_db()
    coop = conn.execute("SELECT * FROM coops WHERE slug = ?", (slug,)).fetchone()
    if not coop:
        conn.close()
        raise HTTPException(status_code=404, detail="Co-op not found")

    membership_types = conn.execute(
        "SELECT * FROM membership_types WHERE coop_id = ? AND active = 1 ORDER BY equity_amount",
        (coop["id"],)
    ).fetchall()
    conn.close()
...
async def submit_signup(request: Request, slug: str):
    ...
    existing = conn.execute(
        "SELECT member_number FROM members WHERE coop_id = ? AND email = ?",
    ...
    mtype = conn.execute(
        "SELECT * FROM membership_types WHERE id = ?", (result["membership_type_id"],)
    ).fetchone()
    conn.close()
...
async def export_copos(slug: str):
    conn = get_db()
    coop = conn.execute("SELECT * FROM coops WHERE slug = ?", (slug,)).fetchone()
    if not coop:
        conn.close()
